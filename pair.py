...
model = PairMultiBlock(M, emb_dim, block_dim, n_layers=n_layers)
model = model.to(device)
print('compiling pair...')
...
for i in range(args.iters):
    train_x, train_y = pair_task(M, batch_size, train_len)
    ...
    if (i+1)%1000 == 0:
        test_x, test_y = pair_task(M, batch_size, test_len)
        test_y = test_y[:,-1]
